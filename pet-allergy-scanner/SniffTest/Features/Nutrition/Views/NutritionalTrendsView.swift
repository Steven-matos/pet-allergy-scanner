        ...
        ScrollView {
            VStack(spacing: ModernDesignSystem.Spacing.lg) {
                ...
                if !trendsService.insights(for: pet.id).isEmpty {
                    InsightsCard(insights: trendsService.insights(for: pet.id), pet: pet)
                } else {
...
struct InsightsCard: View {
    ...
    private func calculateRecommendedDogWeight(ageMonths: Int, breed: String?) -> Double {
        ...
        // For puppies, weight increases with age
        if ageMonths < 12 {
            return calculatePuppyWeight(ageMonths: ageMonths, breedSize: breedSize)
        }
        
        // For adults and seniors, use breed-appropriate weight ranges
        switch breedSize {
        case .small:
            return 5.0 // 5-10 kg range, use midpoint
        case .medium:
            return 15.0 // 10-25 kg range, use midpoint
        case .large:
            return 30.0 // 25-40 kg range, use midpoint
        case .giant:
            return 50.0 // 40-70 kg range, use midpoint
        }
    }
    
    /// Calculate recommended weight for cats based on age
    private func calculateRecommendedCatWeight(ageMonths: Int) -> Double {
        // For kittens, weight increases with age
        if ageMonths < 12 {
            // Kittens typically reach adult weight by 12 months
            // Average adult cat weight is 4-5 kg
            let adultWeight = 4.5
            // Linear growth from 0.5 kg at birth to adult weight
            return min(0.5 + (Double(ageMonths) / 12.0) * (adultWeight - 0.5), adultWeight)
        }
        
        // Adult and senior cats: 3.5-5.5 kg range, use 4.5 kg as ideal
        return 4.5
    }
    ...
    private func calculatePuppyWeight(ageMonths: Int, breedSize: DogBreedSize) -> Double {
        ...
        switch breedSize {
        case .small:
            adultWeight = 7.5 // Average small breed adult weight
        case .medium:
            adultWeight = 17.5 // Average medium breed adult weight
        case .large:
            adultWeight = 32.5 // Average large breed adult weight
        case .giant:
            adultWeight = 55.0 // Average giant breed adult weight
        }
        
        // Puppies typically reach 50% of adult weight by 4 months, 75% by 8 months, 90% by 12 months
